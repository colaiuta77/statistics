...
![BookOasis 통계 대시보드](docs/statistics-dashboard-overview.png?v=1.3.0)

![BookOasis 통계 상세 화면](docs/statistics-dashboard-detail.png?v=1.3.0)
...
- 용량 상위 항목을 포맷별로 묶은 큰 트리맵 카드를 제공하며 기존 용량 막대 카드도 유지합니다.
...
`용량이 큰 도서 · 트리맵`은 [ECharts Disk Usage 예제](https://echarts.apache.org/examples/en/editor.html?c=treemap-disk)를 참고한 포맷 → 항목 구조입니다. 사각형 면적은 파일 크기에 비례하며 제목·용량을 표시합니다. 포맷 영역을 클릭하면 확대되고 아래 경로로 돌아갈 수 있습니다. `보관함별 메타데이터 완성도`와 같은 2열 폭·2단 높이를 사용합니다.

새 쿼리 없이 기존 용량 상위 스냅샷을 사용합니다. 일반·성인은 전체 상위 20개 또는 개별 보관함 상위 15개, 오디오북·비디오는 상위 20개까지 표시합니다. 전체 디스크나 전체 라이브러리의 용량 비중이 아니며, 기존 막대그래프는 별도 카드로 유지합니다.
