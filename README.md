...
![BookOasis 통계 대시보드](docs/statistics-dashboard-overview.png?v=1.4.1)

![BookOasis 통계 상세 화면](docs/statistics-dashboard-detail.png?v=1.4.1)
...
- 일반 카드는 1단, Heatmap·용량 트리맵·장르 연관성은 2단 높이를 사용해 재배치 시 정렬을 맞춥니다. 장르/태그 분포와 용량 막대 카드는 가로 2열 폭·1단 높이입니다.
...
`용량이 큰 도서`는 가로 2열 폭·1단 높이로 용량 상위 최대 50개를 표시합니다. 큰 항목부터 10개씩 보이며 차트 위에서 마우스 휠을 굴리거나 오른쪽 세로 스크롤바를 이동해 나머지를 확인합니다. 항목이 10개 이하면 스크롤바를 숨깁니다. 좁은 화면에서는 1열 폭으로 맞춰집니다.
