...
![BookOasis 통계 대시보드](docs/statistics-dashboard-overview.png?v=1.4.0)

![BookOasis 통계 상세 화면](docs/statistics-dashboard-detail.png?v=1.4.0)
...
- 일반 카드는 1단, Heatmap·용량 트리맵·장르/태그 분포·장르 연관성은 2단 높이를 사용해 재배치 시 정렬을 맞춥니다.
...
- 모든 세션의 전체/개별 보관함에서 용량 상위 최대 100개를 제공합니다. 전체 디스크나 전체 라이브러리의 용량 비중이 아니며, 기존 막대그래프는 별도 카드에서 상위 12개를 유지합니다.
...
분포 카드는 `장르 연관성`과 같은 1열 폭·2단 높이이며 상위 최대 60개를 표시합니다. 일반·성인 도서에서는 카드 오른쪽에서 `장르` 또는 `태그`를 선택하고, 선택은 세션별로 이 브라우저에 저장합니다. 장르 연관성 카드는 기존 장르 기준을 유지합니다.
